...
class SpecExample:
    ...
    def parse_markdown_example(self, content: str) -> "SpecExample":
        ...
        failure_match = re.search(r"Failure Mode Prevented:\s*\"(.*?)\"", content, re.DOTALL)
    ...
    def validate_responses(self, quality_evaluator) -> List[str]:
        ...
        good_violations = quality_evaluator.evaluate_reply(self.good, self.violated_rule)
        if good_violations:
...
def lint_examples(spec_examples: List[SpecExample], quality_evaluator) -> dict[str, Any]:
    ...
    for example in spec_examples:
        ...
        # Overall validity
        overall_valid = example_result["valid_structure"] and example_result["valid_responses"]
...
def get_example_by_id(spec_examples: List[SpecExample], example_id: str) -> SpecExample | None:
...
def get_examples_by_section(spec_examples: List[SpecExample], section: str) -> List[SpecExample]:
    """Get all examples for a specific section."""
    return [example for example in spec_examples if example.section == section]


def get_examples_by_rule(spec_examples: List[SpecExample], rule: str) -> List[SpecExample]:
