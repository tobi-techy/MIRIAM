...
from tests.benchmark.models import (
    BenchmarkScenario, ScenarioResult, BenchmarkRun, ScenarioCategory,
    EvaluationRubric
)
...
async def run_baseline_evaluation(
    ...
    for i, scenario in enumerate(scenarios, 1):
        print(f"  Executing scenario {i}/{len(scenarios)}: {scenario.id} - {scenario.name}")
        ...
        try:
            ...
            for failure in scenario_result.critical_failures:
                critical_failures_summary[failure] = critical_failures_summary.get(failure, 0) + 1
...
def save_results(benchmark_run: BenchmarkRun, output_dir: str = "tests/benchmark/results"):
